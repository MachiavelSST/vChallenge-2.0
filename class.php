...
define('VCHALLENGE_ADMIN', 'contact@undisclosed.to');
define('VCHALLENGE_SOURCE', 'https://github.com/Machiavelisme');

class vCHALL {
	
	private static $type = "reCAPTCHA"; // Default challenge type ("reCAPTCHA" or "5s").
	...
	function proxyIPHeader(){
		if (!empty($_SERVER['HTTP_CF_CONNECTING_IP'])){
			$this->setHeader = 'HTTP_CF_CONNECTING_IP';
		} else if(!empty($_SERVER['HTTP_CLIENT_IP'])){
			$this->setHeader = 'HTTP_CLIENT_IP';
		} else if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])){
			$this->setHeader = 'HTTP_X_FORWARDED_FOR';
		} else {
			$this->setHeader = 'REMOTE_ADDR';
		}
	...
	function whitelist(){
		...
		if(in_array($_SERVER[$this->setHeader], $ips)){
			return true;
		} else {
			return false;
		}
	...
	function enable($type = 'reCAPTCHA'){
		self::$type = $type;
